...
for i in range(num_instances):
    ...
    optimal_fair_alloc = max(best_allocs, key=lambda alloc: calculate_total_welfare(valuations, alloc))
    optimal_fair_alloc_egal = max(best_allocs, key=lambda alloc: egalitarian_welfare(valuations, alloc))

    max_welfare_alloc = max_weighted_matching(valuations)
    max_w = calculate_total_welfare(valuations, max_welfare_alloc)
    ...
    for idx, q in enumerate(qs):
        ...
        fair_w = calculate_total_welfare(valuations, fair_alloc_q)
...
mean_max = np.mean(welfare_max_all) / n_agents
std_max = np.std(welfare_max_all, ddof=1) / n_agents
ci_max = 1.96 * std_max / np.sqrt(num_instances)
ci_max_lower = [mean_max - ci_max] * len(qs)
ci_max_upper = [mean_max + ci_max] * len(qs)
mean_fair = [np.mean(wlist) / n_agents for wlist in welfare_fair_q]
ci_lower = [np.mean(w) / n_agents - 1.96 * np.std(w, ddof=1) / np.sqrt(num_instances) / n_agents for w in welfare_fair_q]
ci_upper = [np.mean(w) / n_agents + 1.96 * np.std(w, ddof=1) / np.sqrt(num_instances) / n_agents for w in welfare_fair_q]
...
plt.plot(qs, [mean_max] * len(qs), label='Average Social Welfare Per Agent', color='blue',linestyle='-', marker='o')
plt.fill_between(qs,ci_max_lower, ci_max_upper, color='blue', alpha=0.2)

# Fair-with-q-changes line + CI band
plt.plot(qs, mean_fair, label='Average Social Welfare after q Reallocations', color='blue',linestyle='--', marker='x')
